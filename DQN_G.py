...
#        state_batch = torch.cat([self.policy_net.forward(state_batch[i]) for i in range(self.batch_size)])
        ...
        reward_batch = torch.from_numpy(np.asarray(batch.reward)).to(self.device)
...
class Env:
    ...
    def step(self, state, action): # state = location of all agents + remaining cp + design variable for fc
        ...
        if easycase:
            ...
            fixed_cost = np.dot(construction, self.fc)
            
            cost = np.dot(extended_uc[action],self.qtt) + fixed_cost
            ...
            cost = shipping_cost.sum() + fixed_cost
